...
def train():
    ...
    tf.flags.DEFINE_string('out_dir',os.path.join(defaultPath.PROJECT_DIRECTORY,sogou_classfication.lstm_model_save_path),'the path of save model')
    ...
    valid_config.w2c = change_gensim_mode2array()
    ...
    with graph.as_default(),tf.Session() as sess:
        ...
        out_dir = os.path.join(defaultPath.PROJECT_DIRECTORY,sogou_classfication.lstm_model_save_path)
...
def train_step(session,model,input_x,input_y,config,summary_writer = None):
    ...
    fetches = [model.train_op,model.global_step,model.learning_rate,model.loss,model.accuracy,model.summary]
