...
class LSTM(object):
    ...
    def __init__(self,config):
        ...
        with tf.name_scope("softmax_layer"):
            softmax_w = tf.get_variable('softmax_w',[self.hidden_layer_num,self.num_classes],dtype=tf.float32)
            softmax_b = tf.get_varibale("softmax_b",[self.num_classes],dtype=tf.float32)
        ...
        self.learning_rate = tf.maximum(tf.train.exponential_decay(self.learning_rate,self.global_step,
                                                                   self.decay_step,self.decay_step,staircase=True),self.min_learning_rate)
